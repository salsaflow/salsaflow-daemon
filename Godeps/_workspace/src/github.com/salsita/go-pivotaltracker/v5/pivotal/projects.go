...
// Next returns the next project.
//
// In case there are no more projects, io.EOF is returned as an error.
func (c *ProjectCursor) Next() (s *Project, err error) {
	if len(c.buff) == 0 {
		_, err = c.next(&c.buff)
		if err != nil {
			return nil, err
...
// Iterate returns a cursor that can be used to iterate over
// the active projects for the current user.
// More projects are fetched on demand.
func (service *ProjectService) Iterate() (c *ProjectCursor, err error) {
	reqfn := func() (req *http.Request) {
		req, _ = service.client.NewRequest("GET", "projects", nil)
		return req
	}
	cc, err := newCursor(service.client, reqfn, 10)
	return &ProjectCursor{cc, make([]*Project, 0)}, err
}
